...
public:
    ...
    friend std::istream& operator>>(std::istream& cin, SquareMatrix& m) {
        ...
        for (int i = 0; i < m.rowsCount; i++) {
            for (int j = 0; j < m.columnsCount; j++) {
                m.matrix.at(i).push_back(i == j ? 1 : 0);
            }
        }
    ...
    friend std::ostream& operator<<(std::ostream& cout, const SquareMatrix& m) {
        for (const auto& item : m.matrix) {
            for (int j = 0; j < item.size(); j++) {
                cout << std::fixed << std::setprecision(2) << item.at(j) << (j + 1 == item.size() ? '\n' : ' ');
            }
        }
    ...
    [[nodiscard]] std::string showAugmented() const {
        std::stringstream output;
    ...
    [[nodiscard]] std::string showInverse() const {
        std::stringstream output;
        output << "Result:\n";

        for (const auto& item : matrix) {
            for (int j = columnsCount; j < item.size(); j++) {
                output << std::fixed << std::setprecision(2) << item.at(j) << ' ';
            }
            output << std::endl;
        }
...
int main() {
    ...
    std::string gaussianOutput, augmentedOutput, inverseOutput;
    try {
        augmentedOutput = m.showAugmented();
        gaussianOutput = m.gaussianElimination();
        inverseOutput = m.showInverse();
    } catch (const std::invalid_argument& exception) {
        std::cout << exception.what();
        return EXIT_FAILURE;
    }

    std::cout << augmentedOutput << gaussianOutput << inverseOutput;
    return EXIT_SUCCESS;
}
